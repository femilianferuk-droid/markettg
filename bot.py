...
async def verify_account(session_string: str = None, phone: str = None) -> dict:
    ...
    Возвращает словарь: {valid: bool, session_string: str, error: str, user_info: dict}
    """
...
async def get_code_from_chat(session_string: str) -> str:
    """Получает код из последнего сообщения в самом новом чате"""
    client = await create_telethon_client(session_string)
    try:
        await client.connect()
        if not await client.is_user_authorized():
...
def main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(
                    text=f"Продать аккаунт"
                ),
                KeyboardButton(
                    text=f"Купить аккаунт"
                ),
            ],
            [
                KeyboardButton(
...
async def sell_account_price(message: Message, state: FSMContext):
    ...
    # Обновляем статус на active после успешной проверки
    cur.execute(
...
async def buy_account_list(message: Message):
...
async def buy_account_confirm(callback: CallbackQuery):
    ...
    # Проверяем валидность сессии перед покупкой
...
async def my_purchases(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text=f"Покупка #{p[0]} | {p[9] if len(p) > 9 else 'Нет данных'}",
...
async def add_balance_start(callback: CallbackQuery):
    ...
    await BuyAccount.waiting_for_code.set_state()
...
async def admin_change_balance_start(callback: CallbackQuery):
    ...
    await AdminStates.waiting_for_user_id.set_state()
...
async def admin_get_user_id(message: Message, state: FSMContext):
    ...
    await AdminStates.waiting_for_amount.set_state()
...
async def back_to_main(callback: CallbackQuery):
    await callback.message.delete()
    await callback.message.answer(
        f"<tg-emoji emoji-id=\"{EMOJI['bot']}\">🤖</tg-emoji> Главное меню",
        reply_markup=main_keyboard()
    )
...
async def back_to_list(callback: CallbackQuery):
    await callback.message.delete()
    await buy_account_list(callback.message)
