...
def get_user_sync(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM users WHERE id = %s::bigint", (user_id,))
            return cur.fetchone()
    finally:
        db_pool.putconn(conn)
...
def create_user_sync(user_id: int, username: str = None):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO users (id, username) VALUES (%s::bigint, %s) ON CONFLICT DO NOTHING",
                (user_id, username)
            )
        conn.commit()
    finally:
...
def update_balance_sync(user_id: int, amount: float) -> float:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "UPDATE users SET balance = balance + %s WHERE id = %s::bigint",
                (amount, user_id)
            )
            cur.execute("SELECT balance FROM users WHERE id = %s::bigint", (user_id,))
            row = cur.fetchone()
        conn.commit()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE accounts 
                SET is_reserved = TRUE, reserved_by = %s::bigint, reserved_until = %s
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE accounts 
                SET is_sold = TRUE, is_reserved = FALSE, sold_to = %s::bigint, buy_price = %s
                WHERE id = %s
                """,
                (user_id, price, account_id)
            )
            cur.execute(
                "INSERT INTO purchases (user_id, account_id) VALUES (%s::bigint, %s)",
                (user_id, account_id)
            )
            cur.execute(
                "UPDATE users SET total_purchases = total_purchases + 1 WHERE id = %s::bigint",
                (user_id,)
            )
        conn.commit()
    finally:
