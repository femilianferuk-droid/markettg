...
def get_unique_countries():
    cur.execute("SELECT DISTINCT country FROM accounts WHERE status = 'active' AND is_valid = TRUE")
...
def buy_account(account_id: int, buyer_id: int):
    cur.execute(
        "UPDATE accounts SET status = 'sold', buyer_id = %s, sold_at = NOW() WHERE id = %s AND status = 'active'",
        (buyer_id, account_id)
    )
    if cur.rowcount > 0:
        cur.execute(
...
async def view_account(callback: CallbackQuery):
    ...
    text = (
        f"<tg-emoji emoji-id=\"{EMOJI['tag']}\">🏷</tg-emoji> <b>{account[3]}</b>\n\n"
    ...
    if seller[5] > 0:
        text += f"\n⭐ Рейтинг: {seller[4]:.1f} ({seller[5]} отзывов)"
...
async def buy_account_confirm(callback: CallbackQuery):
    ...
    try:
        await bot.send_message(
            account[1],
            f"<tg-emoji emoji-id=\"{EMOJI['gift']}\">🎁</tg-emoji> Ваш аккаунт «{account[3]}» был продан!\n"
...
async def view_seller_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        f"⭐ Рейтинг: {seller[4]:.1f}" if seller[5] > 0 else "⭐ Рейтинг: Нет оценок"
    )
    
    if seller[5] > 0:
        text += f" ({seller[5]} отзывов)"
...
async def my_purchases(callback: CallbackQuery):
    ...
    buttons = []
    for p in purchases:
        title = p[11] if len(p) > 11 else f"Покупка #{p[0]}"
        buttons.append([InlineKeyboardButton(
            text=title,
...
async def view_purchase(callback: CallbackQuery):
    ...
    text = (
        f"<tg-emoji emoji-id=\"{EMOJI['box']}\">📦</tg-emoji> <b>{purchase[11]}</b>\n\n"
