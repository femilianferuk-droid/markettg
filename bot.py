        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS price_settings (
                    ...
                    buy_price DOUBLE PRECISION DEFAULT 100.0
                )
            """)
...
# --- Функции БД (синхронные, ПРАВИЛЬНЫЕ) ---
...
def create_user_sync(user_id: int, username: str = None):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO users (id, username) VALUES (%(id)s, %(username)s) ON CONFLICT DO NOTHING",
...
def update_balance_sync(user_id: int, amount: float) -> float:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "UPDATE users SET balance = balance + %(amount)s WHERE id = %(id)s",
                {"amount": amount, "id": user_id}
            )
            cur.execute("SELECT balance FROM users WHERE id = %(id)s", {"id": user_id})
            row = cur.fetchone()
        conn.commit()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE accounts 
                SET is_reserved = TRUE, reserved_by = %(user_id)s, reserved_until = %(time)s
                WHERE id = %(account_id)s AND is_sold = FALSE AND is_valid = TRUE
                """,
                {
                    "user_id": user_id,
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE accounts 
                SET is_sold = TRUE, is_reserved = FALSE, sold_to = %(user_id)s, buy_price = %(price)s
                WHERE id = %(account_id)s
                """,
                {"user_id": user_id, "price": price, "account_id": account_id}
            )
            cur.execute(
                "INSERT INTO purchases (user_id, account_id) VALUES (%(user_id)s, %(account_id)s)",
                {"user_id": user_id, "account_id": account_id}
            )
            cur.execute(
                "UPDATE users SET total_purchases = total_purchases + 1 WHERE id = %(id)s",
                {"id": user_id}
            )
        conn.commit()
    finally:
...
def get_price_sync(country: str) -> float:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "SELECT buy_price FROM price_settings WHERE country = %(country)s",
                {"country": country}
            )
            row = cur.fetchone()
...
def set_price_sync(country: str, value: float):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO price_settings (country, buy_price) VALUES (%(country)s, %(value)s)
                ON CONFLICT (country) DO UPDATE SET buy_price = %(value)s
                """,
...
def get_media_sync(section: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM media_settings WHERE section = %(section)s",
                {"section": section}
            )
            return cur.fetchone()
    finally:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                VALUES (%(section)s, %(file_id)s, %(file_type)s, %(caption)s)
                ON CONFLICT (section) DO UPDATE 
                SET file_id = %(file_id)s, file_type = %(file_type)s, caption = %(caption)s
                """,
                {
                    "section": section,
                    "file_id": file_id,
...
def get_account_by_id_sync(account_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM accounts WHERE id = %(id)s",
                {"id": account_id}
            )
            return cur.fetchone()
    finally:
...
def mark_code_obtained_sync(purchase_id: int):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE purchases SET code_obtained = TRUE WHERE id = %(id)s",
                {"id": purchase_id}
            )
        conn.commit()
    finally:
...
async def callback_admin_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Отмена", callback_data="admin_close", style="danger", icon_custom_emoji_id="5870657884844462243")]
        ])
    )
    ...
    for user in users:
        try:
            if data.get('broadcast_type') == "text" and not data.get('broadcast_file_id'):
                ...
                await bot.send_photo(chat_id=user['id'], photo=data['broadcast_file_id'], caption=data.get('broadcast_text', ''))
            elif data.get('broadcast_file_type') == "video":
                await bot.send_video(chat_id=user['id'], video=data['broadcast_file_id'], caption=data.get('broadcast_text', ''))
            success += 1
...
async def admin_change_balance_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> <b>Неверный ID!</b> Попробуйте снова.")
        return
    user = await get_user(user_id)
    if not user:
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> <b>Пользователь не найден!</b>")
        return
...
async def admin_change_balance_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> <b>Неверная сумма!</b> Попробуйте снова.")
        return
    data = await state.get_data()
...
async def admin_change_price_value(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> <b>Неверная цена!</b> Попробуйте снова.")
        return
    data = await state.get_data()
...
async def callback_admin_upload_media(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        ...
        return
    builder = InlineKeyboardBuilder()
    for section, section_id in [("Главное меню", "main_menu"), ("Профиль", "profile"), ("Мои покупки", "purchases")]:
        builder.row(InlineKeyboardButton(
            text=section,
            callback_data=f"media_section_{section_id}",
            style="default",
            icon_custom_emoji_id="6035128606563241721"
        ))
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="admin_close", style="danger", icon_custom_emoji_id="5870657884844462243"))
...
async def admin_upload_media_message(message: Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> <b>Отправьте фото или видео!</b>")
        return
