...
EMOJI = {
    ...
    "chart_up": "5870930636742595124",   # 📊 Рост график
    ...
    "speaker": "6039422865189638057",    # 📣 Рупор
    ...
    "eye_hide": "6037243349675544634",   # 👁 Скрыто
    ...
    "globe": "6042011682497106307",      # 📍 Геометка
    ...
    "back": "5345906554510012647",       # ◁ Назад
    "star": "5373141891321699086",       # ⭐ Звезда
    "blue_star": "5370810157871667232",  # 🔵 Синяя звезда
    "green_star": "5471984997361523302", # 🟢 Зеленая звезда
    "pin": "5890937706803894250",        # 📌 Закреп
    "resell": "5778672437122045013",     # 🔄 Перепродажа
    "valid": "6037397706505195857",      # 🟢 Валид
    "invalid": "6037243349675544634",    # 🔴 Невалид
    "ban": "5893192487324880883",        # 🚫 Бан
    "unban": "5891207662678317861",      # ✅ Разбан
}
...
def process_expired_holds():
    ...
    for acc in cur.fetchall():
        try:
            ...
            logger.info(f"Холд разморожен: аккаунт #{acc[0]}, сумма {acc[2]} руб")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if data.get("ok"):
                    ...
                    # Сохраняем счёт в БД
                    cur.execute(
                        "INSERT INTO crypto_invoices (user_id, invoice_id, amount_rub, crypto_amount, currency) VALUES (%s,%s,%s,%s,%s)",
                        (0, result["invoice_id"], amount_rub, str(crypto_amount), currency)
                    )
                    return {
                        "success": True,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if data.get("ok") and data["result"]["items"]:
                    ...
                    cur.execute(
                        "UPDATE crypto_invoices SET status=%s WHERE invoice_id=%s",
...
def main_menu():
    ...
    kb.row(InlineKeyboardButton(
        ...
        callback_data="sell",
        icon_custom_emoji_id=EMOJI["send_money"]
    ))
    kb.row(InlineKeyboardButton(
        text="Купить аккаунт",
        callback_data="buy_list",
        icon_custom_emoji_id=EMOJI["receive_money"]
    ))
    kb.row(InlineKeyboardButton(
        text="Профиль",
...
def acc_list_kb(accounts, page=1, total=0):
    ...
    for acc in accounts:
        # acc[17] = is_pinned
        icon = EMOJI["pin"] if acc[17] else EMOJI["globe"]
        kb.row(InlineKeyboardButton(
            text=f"{acc[2]} | {acc[8]} руб",
...
def mylist_kb(accounts):
    ...
    kb = InlineKeyboardBuilder()
    for acc in accounts:
        icon = EMOJI["pin"] if acc[17] else EMOJI["tag"]
        kb.row(InlineKeyboardButton(
            text=f"{acc[2]} | {acc[8]} руб",
...
def list_act_kb(account_id):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(
        text=f"Закрепить ({PIN_PRICE}руб/{PIN_DURATION}мин)",
...
def crypto_curr_kb(amount):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(
        text=f"TON (1 TON = 140 руб)",
        callback_data=f"crypto_TON_{amount}",
        icon_custom_emoji_id=EMOJI["crypto"]
    ))
    kb.row(InlineKeyboardButton(
        text=f"USDT (1 USDT = 90 руб)",
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        f"{em('bot')} Добро пожаловать в Маркетплейс Telegram аккаунтов!\n\n"
...
async def cb_profile(callback: CallbackQuery):
    ...
    rating_text = f"{em('star')} Рейтинг: {rating:.1f}" if total_reviews > 0 else "Нет оценок"
    frozen_text = f"\n{em('lock')} Заморожено: {frozen} руб" if frozen > 0 else ""
    
    await callback.message.edit_text(
        f"{em('profile')} Профиль\n\n"
        f"{em('wallet')} Баланс: {balance} руб"
        f"{frozen_text}\n"
...
async def sell_start(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{em('send_money')} Выберите способ добавления аккаунта:",
...
async def sell_phone_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{em('send_money')} Введите номер телефона в международном формате:\n"
        f"+79991234567",
...
async def sell_file_received(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            f"{em('globe')} Страна определена: {country}\n\n"
...
async def sell_price(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{em('check')} Аккаунт успешно выставлен на продажу!\n\n"
        f"{em('tag')} Название: {data['title']}\n"
        f"{em('money')} Цена: {price} руб\n"
        f"{em('globe')} Страна: {data['country']}\n"
...
async def show_accs(callback: CallbackQuery, page=1):
    ...
    # Показываем список
    await callback.message.edit_text(
        f"{em('receive_money')} Доступные аккаунты\n"
...
async def view_acc(callback: CallbackQuery):
    ...
    if seller[6] > 0:
        text += f"\n{em('star')} Рейтинг: {seller[5]:.1f} ({seller[6]} отзывов)"
...
async def check_buy(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"{em('money')} Цена: {account[8]} руб\n\n"
        f"{em('check')} Аккаунт валиден!\n"
        f"Нажмите Купить для подтверждения покупки.",
...
async def buy_acc(callback: CallbackQuery):
    ...
    if buyer[3] < account[8]:
        await callback.answer(
            f"Недостаточно средств! Ваш баланс: {buyer[3]} руб, требуется: {account[8]} руб",
            show_alert=True
        )
        return
    ...
    purchase_text = (
        f"{em('check')} Аккаунт успешно куплен!\n\n"
        f"ID покупки: {purchase_uid}\n"
        f"{em('tag')} {account[2]}\n"
        f"{em('globe')} Страна: {account[6]}\n"
        f"{em('file')} Номер: {account[3]}\n"
    )
    
    if account[4]:
        purchase_text += f"{em('lock')} 2FA пароль: {account[4]}\n"
    else:
        purchase_text += f"{em('unlock')} 2FA: Отсутствует\n"
    
    purchase_text += (
        f"\n{em('money')} Списано: {account[8]} руб\n"
    ...
    try:
        await bot.send_message(
            ...
            f"ID покупки: {purchase_uid}\n"
            f"{em('money')} Сумма: {account[8]} руб\n"
...
async def seller_prof(callback: CallbackQuery):
    ...
    text = (
        f"{em('people')} Профиль продавца\n\n"
        f"Username: {seller[2] or 'Скрыт'}\n"
    )
    
    if seller[6] > 0:
        text += f"{em('star')} Рейтинг: {seller[5]:.1f} ({seller[6]} отзывов)"
    else:
        text += f"{em('star')} Рейтинг: Нет оценок"
    ...
    if reviews:
        text += f"\n\n{em('edit')} Последние отзывы:"
        for review in reviews[:5]:
            text += f"\n- {review[0]} звёзд от пользователя {review[2]}"
...
async def filters_menu(callback: CallbackQuery):
    ...
    filter_text = (
        f"{em('chart_up')} Фильтры поиска:\n\n"
        f"Страна: {filters.get('country', 'Все')}\n"
        f"Цена от: {filters.get('pf', 'Нет')} руб\n"
        f"Цена до: {filters.get('pt', 'Нет')} руб\n"
...
async def valid_all(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{em('stats')} Результаты проверки:\n\n" + "\n".join(results),
        reply_markup=back_kb()
    )
...
async def view_purch(callback: CallbackQuery):
    ...
    text = (
        f"{em('box')} {purchase[11]}\n\n"
        f"ID: {purchase[1]}\n"
        f"{em('globe')} Страна: {purchase[9]}\n"
        f"{em('info')} Описание: {purchase[10] or 'Нет'}\n\n"
        f"{em('file')} Номер: {purchase[5]}\n"
    )
    
    if purchase[6]:
        text += f"{em('lock')} 2FA пароль: {purchase[6]}\n"
    else:
        text += f"{em('unlock')} 2FA: Отсутствует\n"
...
async def rate(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Вы поставили оценку: {rating} {em('star')}",
        reply_markup=back_kb()
    )
    
    logger.info(f"Пользователь {callback.from_user.id} оставил отзыв {rating} звёзд на покупку #{purchase_id}")
...
async def resell_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{em('resell')} Перепродажа аккаунта\n\n"
...
async def resell_price(message: Message, state: FSMContext):
    ...
    if success:
        await message.answer(
            ...
            f"{em('money')} Новая цена: {price} руб",
            reply_markup=main_menu()
        )
...
async def pin_acc(callback: CallbackQuery):
    ...
    if user[3] < PIN_PRICE:
        await callback.answer(
            f"Недостаточно средств! Нужно {PIN_PRICE} руб, у вас: {user[3]} руб",
            show_alert=True
        )
        return
...
async def bal_amount(message: Message, state: FSMContext):
    ...
    await message.answer_invoice(
        ...
        currency="XTR",
        prices=[LabeledPrice(label=f"{amount} руб", amount=amount * STARS_RATE)],
        provider_token="",
    )
    
    await state.clear()
    logger.info(f"Пользователь {message.from_user.id} запросил пополнение на {amount} руб через звёзды")
...
async def success_pay(message: Message):
    ...
    if payload.startswith("bal_"):
        ...
        await message.answer(
            f"{em('check')} Баланс пополнен!\n\n"
            f"{em('money')} Зачислено: {amount} руб\n"
            f"{em('wallet')} Текущий баланс: {user[3]} руб",
            reply_markup=main_menu()
        )
        
        logger.info(f"Пользователь {message.from_user.id} пополнил баланс на {amount} руб через звёзды")
...
async def adm_uid(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{em('profile')} Пользователь найден:\n\n"
        f"ID: {user[1]}\n"
        f"Username: {user[2] or 'Не указан'}\n"
        f"{em('wallet')} Баланс: {user[3]} руб\n"
        f"{em('lock')} Заморожено: {user[4]} руб\n"
...
async def adm_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{em('check')} Баланс изменён!\n\n"
        f"Пользователь: {user[2] or user_id}\n"
        f"Изменение: {'+' if amount > 0 else ''}{amount} руб\n"
        f"{em('wallet')} Новый баланс: {user[3]} руб",
        reply_markup=main_menu()
    )
    
    logger.info(f"Админ {message.from_user.id} изменил баланс пользователя {user_id} на {amount} руб")
    await state.clear()
...
async def adm_users(callback: CallbackQuery):
    ...
    for user in users[:20]:
        ban_status = "BAN" if user[7] else "OK"
        username = user[2] or f"ID:{user[1]}"
        kb.row(InlineKeyboardButton(
            text=f"{ban_status} {username} | {user[3]} руб",
...
async def adm_user_view(callback: CallbackQuery):
    ...
    text = (
        f"{em('profile')} Профиль пользователя\n\n"
        f"Telegram ID: {user[1]}\n"
        f"Username: {user[2] or 'Не указан'}\n"
        f"{em('wallet')} Баланс: {user[3]} руб\n"
        f"{em('lock')} Заморожено: {user[4]} руб\n"
        f"{em('star')} Рейтинг: {user[5]:.1f} ({user[6]} отзывов)\n"
        f"Статус: {'Заблокирован' if user[7] else 'Активен'}"
    )
...
async def adm_stat(callback: CallbackQuery):
    ...
    stats_text = (
        ...
        f"{em('money')} Всего пополнено: {total_deposits} руб\n"
        f"{em('lock')} Заморожено средств: {total_frozen} руб"
    )
